 ...
 * 2）尽量使用使用相同的布局
 * 3）尽量减少布局层次，复杂的布局使用ConstraintLayout（不要过度依赖ConstraintLayout，一般简单布局RelativeLayout性能更高）
 * <p>
...
public class UITableView extends RecyclerView {
    ...
    public void onChildAttachedToWindow(View child) {
        ...
        resetCellSelectionStyle(child);
    }
    
    private void resetCellSelectionStyle(View child) {
    ...
    public void setTableViewDataSource(UITableViewDataSource tableViewDataSource) {
        if (mTableViewDataSource != tableViewDataSource) {
            ...
            if (mTableViewDataSource == null && mTableViewAdapter != null) {
                mTableViewAdapter = null;
                super.setAdapter(null);
            } else if (mTableViewDataSource != null && mTableViewAdapter == null) {
                mTableViewAdapter = new UITableViewAdapter(this);
                super.setAdapter(mTableViewAdapter);
            }
        }
    ...
    public boolean isSwipeActionEnabled() {
        return mOnItemSwipeListener != null;
    }
    
    boolean canBeSwiped() {
        // cell should not be swiped while in editing state
    ...
    public void setSwipeDelegate(@Nullable UITableViewDelegate.SwipeDelegate swipeDelegate) {
        if (mSwipeDelegate != swipeDelegate) {
            mSwipeDelegate = swipeDelegate;
            if (mOnItemSwipeListener != null) {
                // remove old swipe listener
                this.removeOnItemTouchListener(mOnItemSwipeListener);
                mOnItemSwipeListener = null;
            }
            if (mSwipeDelegate != null) {
                mOnItemSwipeListener = new OnItemSwipeListener(this, mSwipeDelegate);
                this.addOnItemTouchListener(mOnItemSwipeListener);
            }
        }
    }
    
    void willBeginSwipingCell(UITableViewCell capturedCell, @UITableViewCell.SwipeLocation int location) {
        capturedCell.showSwipeActionLayout(location);
        capturedCell.willStartSwiping(location);
        
        this.setStateMask(UITableViewCell.StateMask.ShowingSwipeAction);
        mSwipeDelegate.willBeginSwiping(this, indexPathForCell(capturedCell));
    }
    
    void willEndSwipingCell(final UITableViewCell capturedCell) {
        this.setStateMask(UITableViewCell.StateMask.Normal);
    }
    ...
    //     if (mOnItemSwipeListener != null) {
    //         mOnItemSwipeListener.closeLastSwipedCell();
    //     }
    // }
    
    public void setDisallowParentInterceptWhenSwiping(boolean needsDisallow) {
        if (mOnItemSwipeListener != null) {
            mOnItemSwipeListener.setNeedsDisallowParentIntercept(needsDisallow);
        }
    }
    ...
    /// Reload Cells At Specified IndexPaths
