...
public class UITableViewAdapter extends RecyclerView.Adapter<UITableViewBaseHolder> {
    ...
    @Override
    public void onViewAttachedToWindow(UITableViewBaseHolder holder) {
        final NSIndexPath indexPath = holder.getNowIndexPath();
        if (indexPath != null) {
            Log.i(TAG, "onViewAttachedToWindow: #### indexPath = " + indexPath.toString());
        }
    }

    @Override
    public void onViewDetachedFromWindow(UITableViewBaseHolder holder) {
        final NSIndexPath indexPath = holder.getNowIndexPath();
        if (indexPath != null) {
            Log.i(TAG, "onViewDetachedFromWindow: ==== indexPath = " + indexPath.toString());
        }
    }
    ...
    @NonNull
    @Override
    public UITableViewBaseHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        //Log.d(TAG, "onCreateViewHolder: ");
        // 初始化view, 这个方法只有在需要新的ViewHolder的时候才会调用，
        // 1）没有发现可以复用的CellView，2）在有动画的时候也会重新创建holder
        // 每个ViewType对应一个ItemView
        if (isHeaderViewType(viewType)) {
            View itemView = mTableViewDataSource.onCreateSectionHeaderView(mTableView, viewType);
            return mTableView.newSectionHeaderHolder(itemView);
        } else if (isFooterViewType(viewType)) {
            View itemView = mTableViewDataSource.onCreateSectionFooterView(mTableView, viewType);
    ...
    @Override
    public void onBindViewHolder(@NonNull UITableViewBaseHolder holder, int position) {
    ...
    @Override
    public void onBindViewHolder(@NonNull UITableViewBaseHolder holder, int position, @NonNull List<Object> payloads) {
        if (payloads.isEmpty()) {
    ...
    void configureEditingForCell(final UITableViewCell tableViewCell, final NSIndexPath indexPath, boolean animated) {
        if (mTableView.isEditing()) {
            ...
            if (canEdit) {
                tableViewCell.setEditing(true, animated);
